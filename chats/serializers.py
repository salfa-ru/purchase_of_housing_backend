...
# class ZhatSerializer(serializers.ModelSerializer):
#     """Сериализатор для получения списка переписок"""
#     msg_id = serializers.IntegerField()
#     realty = RealtyForZhatSerializer()
#     is_new = serializers.SerializerMethodField()
#     # is_blocked = serializers.SerializerMethodField()  # 2025-Feb-04 - добавляем инфу о блокировке 4ата
...
#             'message',
#             'created_at',
#             # 'is_blocked',  # 2025-Feb-04 - добавляем инфу о блокировке 4ата
...
class MessageSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'message',
            'created_at',
            'direction',  # True если сообщение входящее дял текущего пользвтателя
            'is_new'
        ]
...
# TODO - не понимаю, почему его пришлось УБРАТЬ - ЗДЕСЬ ОТЛИЧНЫЙ ВЫВОД В АПИ!
# class ChatMessagesSerializer(serializers.ModelSerializer):
#     """Сериализатор для отображения чата с сообщениями"""
#     chat_id = serializers.IntegerField(source='chat.chat_id')
#     i_am = serializers.SerializerMethodField()
#     they_are = serializers.SerializerMethodField()
