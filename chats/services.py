...
def get_chat_by_chat_id(user, chat_id):
    ...
    chat = (
        Chat.objects.filter(Q(owner=user) | Q(client=user))
...
def check_blocking(user_from, user_to):
    ...
    is_blocked_by_other = Blocking.objects.filter(
...
def create_message(user_from, message_text, realty_id=None, chat_id=None):
    ...
    if chat_id is not None:
        ...
        chat, created = Chat.objects.get_or_create(
            realty=realty, owner=realty.owner, client=user_from
        )
...
def get_users_from_chats(current_user, chat_ids):
    ...
    if invalid_chat_ids:
        raise exceptions.NotFound(
            f'You are not a member of chats with ids: {invalid_chat_ids}'
        )
...
def get_chats_from_users(current_user, users_to_operate):
    ...
    chats = (
        ...
        .filter(Q(owner__in=users_to_operate) | Q(client__in=users_to_operate))
        .distinct()
    )
