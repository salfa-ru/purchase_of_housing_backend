...
class ComplaintsSerializer(serializers.ModelSerializer):
    realty_id = serializers.PrimaryKeyRelatedField(
        queryset=Realty.objects.all(), source='realty', write_only=True
    )
    ...
    def validate_description(self, value):
        if not (50 <= len(value) <= 200):
            raise serializers.ValidationError(
                'Описание должно содержать от 50 до 200 символов.'
            )
        return value
    ...
    def validate(self, attrs):
        ...
        if realty.owner == owner:
            ...
            raise serializers.ValidationError(
                'Вы не можете отправить жалобу если объявление не активно!'
            )
