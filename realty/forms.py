    ...
    def set_status_choices(self):
        ...
        if not self.request.user.is_superuser:
            ...
            if current_status.status == 'На модерации':
                allowed_statuses = RealtyAdvStatus.objects.filter(
                    status__in=['Активно', 'Отклонено']
                )
            ...
            self.fields['realty_status'].queryset = RealtyAdvStatus.objects.filter(
                id__in=[current_status.id]
                + list(allowed_statuses.values_list('id', flat=True))
            )
    ...
    def set_restricted_status_help_text(self):
        ...
        if current_status == 'На модерации':
            self.fields['realty_status'].help_text = (
                'Вы можете изменить статус только на '
            ...
            self.fields[
                ...
                "Вы можете изменить статус только на <strong>'Отклонено'</strong>."
            )
        ...
        if self.request.user.is_superuser:
            self.fields['realty_status'].help_text = (
                'При данном статусе модератор увидит следующее сообщение: <br>'
                + self.fields['realty_status'].help_text
            )
            ...
            if old_instance.realty_status.status == 'На модерации':
                if realty_status.status not in ['Активно', 'Отклонено']:
                    raise forms.ValidationError(
                        "Статус можно изменить только на 'Активно' или 'Отклонено'."
                    )
            elif old_instance.realty_status.status == 'Активно':
                if realty_status.status != 'Отклонено':
                    raise ValidationError(
                        "Статус можно изменить только на 'Отклонено', если он 'Активно'."
                    )
