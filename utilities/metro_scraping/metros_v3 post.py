...
DADATA_SUGGEST_LIMIT = 20  # Максимальное количество подсказок для бесплатного тарифа Dadata
...
def fetch_stations_recursive(query_prefix, current_depth=0, max_depth=1):
    ...
    try:
        ...
        if suggestions:
            ...
            print(f"  Получено {actual_moscow_count} московских станций для префикса '{query_prefix}'.")

            if actual_moscow_count == DADATA_SUGGEST_LIMIT and current_depth < max_depth:
                print(f"  Достигнут лимит для '{query_prefix}'. Углубляемся...")
                for next_letter in russian_alphabet:
                    time.sleep(0.3)
                    fetch_stations_recursive(query_prefix + next_letter, current_depth + 1, max_depth)
                ...
                print(f"  Достигнута максимальная глубина рекурсии для префикса '{query_prefix}'.")
        else:
            print(f"  Станций на префикс '{query_prefix}' не найдено (ответ API пуст или нет поля 'suggestions').")

    except requests.exceptions.HTTPError as http_err:
        print(f"  HTTP ошибка при запросе для префикса '{query_prefix}': {http_err} - {response.text}")
    except requests.exceptions.ConnectionError as conn_err:
        print(f"  Ошибка соединения при запросе для префикса '{query_prefix}': {conn_err}")
        ...
        print(f"  Ошибка декодирования JSON ответа для префикса '{query_prefix}'. Ответ: {response.text if 'response' in locals() else 'N/A'}")
...
try:
    ...
    print(f"Загружено {len(loaded_raw_stations_from_file)} сырых записей для обработки.")
except FileNotFoundError:
    print(f"Ошибка: Файл с сырыми данными '{raw_input_filename}' не найден. "
          "Убедитесь, что первая часть скрипта успешно отработала.")
    exit()
except json.JSONDecodeError:
    print(f"Ошибка: Не удалось декодировать JSON из файла '{raw_input_filename}'. Файл может быть поврежден.")
    exit()
except IOError as e:
...
if not loaded_raw_stations_from_file:
    print("Файл с сырыми данными пуст или не удалось загрузить данные. Обработка прекращена.")
    exit()
...
for station_entry in loaded_raw_stations_from_file:
    ...
    if 'data' in station_for_signature_creation and isinstance(station_for_signature_creation['data'], dict):
    ...
    station_signature = json.dumps(station_for_signature_creation, sort_keys=True, ensure_ascii=False)

    if station_signature not in seen_station_signatures:
        seen_station_signatures.add(station_signature)
        unique_stations_list.append(station_for_signature_creation) # Добавляем уже "очищенную" версию

print(f"После удаления дубликатов осталось {len(unique_stations_list)} уникальных станций.")

# Формирование итогового JSON с уникальными станциями
output_data_unique_stations = {
    "moscow_metro_stations": unique_stations_list
}
...
try:
    ...
    print(f"Уникальные данные станций успешно сохранены в файл: {unique_stations_output_filename}")
except IOError as e:
...
def generate_normalized_db_files(stations_input_list):
    ...
    alphabetically_sorted_stations = sorted(stations_input_list,
                                            key=lambda s: s.get('value', '').lower())
    ...
    for i, station_entry in enumerate(alphabetically_sorted_stations):
        ...
        if line_fk is None and original_station_line_id is not None:
            print(f"Предупреждение: Не найден PK для line_id '{original_station_line_id}' "
...
if 'unique_stations_list' in locals() and isinstance(unique_stations_list, list) and unique_stations_list:
