...
При создании Pull Request в ветку `dev` автоматически запускаются проверки:

| Проверка | Инструмент | Что делает |
|----------|------------|-------------|
| Линтер | Ruff | Проверяет стиль кода, неиспользуемые импорты, потенциальные баги |
| Форматтер | Ruff | Проверяет единый стиль форматирования (отступы, длина строк) |
| Безопасность | Bandit | Ищет уязвимости и потенциально опасные конструкции |

Статус проверок отображается внизу Pull Request:
