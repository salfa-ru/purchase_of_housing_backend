...
Ранее база наполнялась не только тестовыми значениями, но и примерами объявлений,
и несколькими "образцовыми" пользователям (вот про старое наполнение данными <sub>[старое наполнение](X_CLEANUP%2FORIGINAL_FIXTURES_2024_TO_BE_DELETED%2FDATA-POPULATION-OLD.md)</sub> - возможно, его можно исаользовать для локальной разработки - чтобы были некоторые данные о пользователях и обьявлениях) <br><br>
НО. Теперь база основная БД наполняется только важными <br>
[Команды наполнения](README-Population.md)
...
Для включения функций деактивации объявлений (на сервере это включено в entrypoint.sh):
```bash
...
## ОЧЕНЬ ВАЖНО!!!
### Создание пользователей будет работать только при включенном режиме DEBUG
Смотри `if DEBUG:` в файлах `config/urls.py` и `users/urls.py`
